...
// Two Vapi defaults broke number dictation: Deepgram spells numbers as words without `numerals`,
// and 0.5s number endpointing splits each digit group into a turn. Neither shows until a real call.
...
describe("transcriber number handling", () => {
  ...
  it("keeps smartFormat on — numerals replaces neither addresses nor dates", () => {
    // smartFormat alone doesn't give digits; the two are complementary, and
    // dropping either regresses a different part of the transcript.
    const t = buildAssistantPayload(config()).transcriber;
    expect(t?.smartFormat).toBe(true);
  });

  it("sets numerals for multilingual agents too, not just English", () => {
    // Gating numerals to "en" like keyterm would leave multilingual callers
    // reciting their number twice.
